...
class SmithingRegistry {
    ...
    fun registerTemplate(
        ...
        val workingTemplate =
            WorkingTemplate(identifier, firstTech, secondTech, thirdTech)
    ...
    fun registerDeformation(rawItem: String, deformedItem: String) {
        FrogsForgingMod.logger?.debug("%s will become %s when processed".format(rawItem, deformedItem))
    }
    ...
    companion object {
        fun getInstance(): SmithingRegistry {
            return FrogsForgingMod.smithingRegistry
        }
    }
}