package com.hoborific.cryo.frogsforging.config
    ...
    fun initializeRecipes(e: FMLPreInitializationEvent) {
        ...
        if (configTemplates is JsonObject) {
            configTemplates.entrySet().forEach processTemplate@{
                ...
                FrogsForgingMod.smithingRegistry.registerTemplate(
                    identifier,
        ...
        if (configRecipes is JsonArray) {
            configRecipes.forEach processRecipe@{
                ...
                val workTechs =
                    FrogsForgingMod.smithingRegistry.getTemplateWithIdentifier(template) ?: return@processRecipe

                FrogsForgingMod.smithingRegistry.addRecipe(inputStackStr, outputStackStr, workTechs)
            }
        }
        ...
        if (deformationMap is JsonObject) {
            deformationMap.entrySet().forEach processDeformation@{
                ...
                FrogsForgingMod.smithingRegistry.registerDeformation(it.key, it.value.asString)
            }
        }
    }
}
